...
if errorlevel 1 (
    ...
    REM Используем PowerShell для получения правильной даты и времени
    set commit_msg=Auto update
    for /f "delims=" %%i in ('powershell -Command "Get-Date -Format \"yyyy-MM-dd HH:mm:ss\"" 2^>nul') do set commit_msg=Auto update: %%i
    
    REM Если PowerShell не работает, используем простое сообщение
    if "%commit_msg%"=="Auto update" (
        REM Пробуем использовать встроенную дату Windows
        set commit_msg=Auto update: %date% %time%
        REM Убираем лишние символы
